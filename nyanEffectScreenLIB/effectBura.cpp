...
BOOL CEffectBura::SetParam(LPVOID lpEffect, int paraKosuu, int* paraPtr,int layer)
{
	...
	if (paraKosuu<2) para[1] = screenSizeX / 2;	//終了中心座標
	if (paraKosuu<3) para[2] = screenSizeY / 2;
	if (paraKosuu<4) para[3] = 105;	//end percent拡大
	if (paraKosuu<5) para[4] = para[3];	//start percent
	if (paraKosuu<6) para[5] = 20;	//end percent透明
	if (paraKosuu<7) para[6] = para[5];	//start percent

	if (paraKosuu<8) para[7] = 0;	//拡大type
	if (paraKosuu<9) para[8] = 0;	//かくだいさぶ

	if (paraKosuu<10) para[9] = 0;	//透明type
	if (paraKosuu<11) para[10] = 0;	//透明さぶ
	...
	if (paraKosuu<12) para[11] = para[1];	//開始中心座標
