...
#include "effectGeoBoxAdd.h"


CEffectGeoBoxAdd::CEffectGeoBoxAdd(CAllEffect* lpAll) : CCommonEffect(lpAll)
{

}

CEffectGeoBoxAdd::~CEffectGeoBoxAdd()
{
	End();
}

void CEffectGeoBoxAdd::End(void)
{
}

BOOL CEffectGeoBoxAdd::SetParam(LPVOID lpEffect, int paraKosuu, int* paraPtr,int layer)
{
...
//	if (paraKosuu<21) para[20] = para[18];	//rx2Start	(ana)
	...
	lp->count = 0;
	lp->countMax = para[0];
	lp->command = EFFECT_GEOBOXADD;

	return TRUE;
}


BOOL CEffectGeoBoxAdd::CountIsMax(LPVOID lpEffect,int layer)
{
	return FALSE;
}


void CEffectGeoBoxAdd::BeginPrint(LPVOID para,int layer)
{

}

void CEffectGeoBoxAdd::Init(LPVOID para,int layer)
{
}

void CEffectGeoBoxAdd::Calcu(LPVOID lpEffect,int layer)
{
}


void CEffectGeoBoxAdd::Print(LPVOID lpEffect,int layer)
{
	EFFECT* lp = (EFFECT*)lpEffect;
	int count = lp->count;
...
BOOL CEffectGeoBoxAdd::CheckAllPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}

BOOL CEffectGeoBoxAdd::CheckNeedPicPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}
