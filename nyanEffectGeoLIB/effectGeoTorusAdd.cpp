//
// effectGeoTorusAdd.cpp
//

#include "..\nyanEffectLib\includer.h"
#include "effectGeoTorusAdd.h"


CEffectGeoTorusAdd::CEffectGeoTorusAdd(CAllEffect* lpAll) : CCommonEffect(lpAll)
{

}

CEffectGeoTorusAdd::~CEffectGeoTorusAdd()
{
	End();
}

void CEffectGeoTorusAdd::End(void)
{
}

BOOL CEffectGeoTorusAdd::SetParam(LPVOID lpEffect, int paraKosuu, int* paraPtr,int layer)
{
	...
	lp->count = 0;
	lp->countMax = para[0];
	lp->command = EFFECT_GEOTORUSADD;

	return TRUE;
}


BOOL CEffectGeoTorusAdd::CountIsMax(LPVOID lpEffect,int layer)
{
	return FALSE;
}


void CEffectGeoTorusAdd::BeginPrint(LPVOID para,int layer)
{

}

void CEffectGeoTorusAdd::Init(LPVOID para,int layer)
{
}

void CEffectGeoTorusAdd::Calcu(LPVOID lpEffect,int layer)
{
}


void CEffectGeoTorusAdd::Print(LPVOID lpEffect,int layer)
{
	EFFECT* lp = (EFFECT*)lpEffect;
	int count = lp->count;
...
//	CMyGraphics::TransCircleFill(cx1,cy1,rx1,ry1,colR,colG,colB);
	CAllGeo::AddTorusFill(centerX,centerY,rx,ry,centerX2,centerY2,rx2,ry2,dr,dg,db,colA);
}


BOOL CEffectGeoTorusAdd::CheckAllPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}

BOOL CEffectGeoTorusAdd::CheckNeedPicPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}
