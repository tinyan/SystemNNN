...
#include "effectGeoCircleAdd.h"


CEffectGeoCircleAdd::CEffectGeoCircleAdd(CAllEffect* lpAll) : CCommonEffect(lpAll)
{

}

CEffectGeoCircleAdd::~CEffectGeoCircleAdd()
{
	End();
}

void CEffectGeoCircleAdd::End(void)
{
}

BOOL CEffectGeoCircleAdd::SetParam(LPVOID lpEffect, int paraKosuu, int* paraPtr,int layer)
{
	...
	lp->count = 0;
	lp->countMax = para[0];
	lp->command = EFFECT_GEOCIRCLEADD;

	return TRUE;
}


BOOL CEffectGeoCircleAdd::CountIsMax(LPVOID lpEffect,int layer)
{
	return FALSE;
}


void CEffectGeoCircleAdd::BeginPrint(LPVOID para,int layer)
{

}

void CEffectGeoCircleAdd::Init(LPVOID para,int layer)
{
}

void CEffectGeoCircleAdd::Calcu(LPVOID lpEffect,int layer)
{
}


void CEffectGeoCircleAdd::Print(LPVOID lpEffect,int layer)
{
	EFFECT* lp = (EFFECT*)lpEffect;
	int count = lp->count;
	...
	CAllGeo::AddCircleFill(centerX,centerY,rx,ry,dr,dg,db,colA);
//	CAllGeo::TransCircleFill(cx1,cy1,rx1,ry1,colR,colG,colB,100);
}


BOOL CEffectGeoCircleAdd::CheckAllPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}

BOOL CEffectGeoCircleAdd::CheckNeedPicPrint(LPVOID lpEffect,int layer)
{
	return TRUE;
}
