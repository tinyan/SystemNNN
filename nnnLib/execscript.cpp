...
//				m_callBack->ReceiveScriptCommand(code,1,(LPVOID)(&m_data[m_pc+4]));
			...
			if (code == CODE_SYSTEMFUNCTION_SETVOICEFLAG)
			{
				m_callBack->ReceiveScriptCommand(code,m_data[m_pc+3],(LPVOID)(&m_data[m_pc+4]));
				m_pc += m_data[m_pc];
				continue;
			...
			if (code == CODE_SYSTEMFUNCTION_SETTERM)
			{
				m_callBack->ReceiveScriptCommand(code,m_data[m_pc+3],(LPVOID)(&m_data[m_pc+4]));
				m_pc += m_data[m_pc];
				continue;
			...
			if (code == CODE_SYSTEMFUNCTION_MESSAGEEFFECTTIME)
			{
				m_callBack->ReceiveScriptCommand(code,m_data[m_pc+3],(LPVOID)(&m_data[m_pc+4]));
				m_pc += m_data[m_pc];
				continue;
...
#if defined _WIN64
#pragma message("***実装したにゃ ここにc++実装が必要にゃ 実装したにゃ" __FILE__)
